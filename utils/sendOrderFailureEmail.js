...
/**
 * Send consolidated email notification on Secondary Order Entry import failures
 * @param {Object} failureSummary - Summary of import failures
 * @param {Array} failureSummary.orderFailures - Array of order failures
 * @param {number} failureSummary.totalProcessed - Total orders processed
 ...
 */
const sendOrderFailureEmail = async (failureSummary) => {
  try {
    const { orderFailures, totalProcessed, totalSuccess, totalFailed } =
      failureSummary;
    ...
    const hasFailures = orderFailures.length > 0;

    if (!hasFailures) {
      return; // Don't send email if there are no failures
    }

    const subject = `⚠️ Secondary Order Entry Failures - ${totalFailed} Failed`;

    const generateFailureRows = (failures) => {
      return failures
        .map(
          (failure, index) => `
        <tr>
          ...
          <td style="padding: 14px 15px; border-bottom: 1px solid #dee2e6; color: #dc3545; font-size: 13px; font-family: monospace; word-break: break-word; max-width: 400px;">
...
<head>
  ...
  <title>Secondary Order Entry Failure Summary</title>
</head>
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f5f5f5;">
  <table role="presentation" style="width: 100%; border-collapse: collapse; background-color: #f5f5f5;">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #dc3545 0%, #c82333 100%); padding: 30px; text-align: center;">
              <h1 style="margin: 0; color: #000000; font-size: 24px; font-weight: 600;">
                ⚠️ Secondary Order Entry Failures
              </h1>
          ...
          <tr>
            <td style="padding: 30px;">
              ...
              <p style="margin: 0 0 20px 0; color: #666666; font-size: 15px; line-height: 1.6;">
                The following orders failed during the Secondary Order Entry import process:
              </p>
              ...
              <h3 style="margin: 30px 0 15px 0; color: #333333; font-size: 18px; font-weight: 600;">
                📦 Failed Orders (${orderFailures.length})
              </h3>
              <table role="presentation" style="width: 100%; border-collapse: collapse; margin: 15px 0; background-color: #f8f9fa; border-radius: 6px;">
                ...
                <thead>
                  <tr style="background-color: #e9ecef;">
                    ...
                    <th style="padding: 14px 15px; text-align: left; font-size: 13px; font-weight: 600; color: #495057; border-bottom: 2px solid #dee2e6;">Order ID</th>
                ...
                <tbody>
                  ${generateFailureRows(orderFailures)}
                </tbody>
              </table>
    ...
    await sendEmail({
      email: "Raju.it@rupa.co.in,rohit.it@rupa.co.in,Bikram.it@rupa.co.in",
      subject,
      htmlMessage,
    });
  } catch (emailError) {
    console.error(
      `Failed to send order entry failure summary email:`,
      emailError.message,
    );
  }
};

module.exports = sendOrderFailureEmail;
