...
const paginatedDistributorTransaction = asyncHandler(async (req, res) => {
  try {
    ...
    // ----------------------------------
    // Retailer Phone, Outlet Code & State filter
    // ----------------------------------
    if (retailerPhone || outletCode || stateId) {
      const outletQuery = {};
    ...
    // ----------------------------------
    if (!retailerPhone && !outletCode && !stateId && retailerId) {
    ...
    if (fromDate || toDate) {
      const dateField =
        dateFilterType === "created" ? "createdAt" : "updatedAt"; // 🔹 NEW
      ...
      if (fromDate) {
        ...
        filter[dateField].$gte = startOfDay;
      }
    ...
    const transactions = await DistributorTransaction.find(filter)
      ...
      .limit(limit)
      .sort({
        [dateFilterType === "created" ? "createdAt" : "updatedAt"]: -1,
      })
