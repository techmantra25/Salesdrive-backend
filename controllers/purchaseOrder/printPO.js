...
const printPO = asyncHandler(async (req, res) => {
  try {
    ...
    const purchaseOrder = await PurchaseOrder.findById(
      ...
      { path: "lineItems.plant", select: "" },
      { path: "selectedBrand", select: "" },
      { path: "selectedPlant", select: "" },
    ]);
    ...
    console.error("PO HTML Generation Error:", {
    ...
    const errorHtml = `
      ...
      <html lang="en">
        <head>
          ...
          <title>Purchase Order Print Preview Generation Error</title>
          <style>
            body {
        ...
        <body>
          <div class="error-container">
            <div class="error-header">
              <h1>⚠️ Purchase Order Print Preview Generation Failed</h1>
            </div>
            <div class="error-content">
              <p>We encountered an error while generating your purchase order print preview. Please try again or contact support if the issue persists.</p>
              
              <div class="error-details">
                <p><strong>Error Type:</strong> ${
                  pdfError.name || "PO Generation Error"
                }</p>
                <p><strong>Message:</strong> ${pdfError.message}</p>
                <p><strong>Purchase Order ID:</strong> ${
