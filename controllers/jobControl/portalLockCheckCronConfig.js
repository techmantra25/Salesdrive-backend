...
const getPortalLockCheckCronConfig = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({
    status: 200,
    error: false,
    message: "Portal lock check cron config fetched successfully",
    data: config,
  });
});

const updatePortalLockCheckCronConfig = asyncHandler(async (req, res) => {
  ...
  if (cronTime === undefined) {
    updateQuery.$setOnInsert.cronTime = PORTAL_LOCK_CHECK_DEFAULT_CRON;
  }
  ...
  await applyPortalLockCheckCronConfig(config);

  return res.status(200).json({
    status: 200,
    error: false,
    message: "Portal lock check cron config updated successfully",
    data: config,
  });
});
