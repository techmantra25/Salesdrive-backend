...
const getPartiallyDeliveredBillRetryCronConfig = asyncHandler(
  async (req, res) => {
    ...
    return res.status(200).json({
      ...
      error: false,
      message:
        "Partially-delivered bill retry cron config fetched successfully",
      data: config,
    });
  },
);

const updatePartiallyDeliveredBillRetryCronConfig = asyncHandler(
  async (req, res) => {
    ...
    if (cronTime === undefined) {
      updateQuery.$setOnInsert.cronTime =
        PARTIALLY_DELIVERED_BILL_RETRY_DEFAULT_CRON;
    }
    ...
    await applyPartiallyDeliveredBillRetryCronConfig(config);

    return res.status(200).json({
      status: 200,
      error: false,
      message:
        "Partially-delivered bill retry cron config updated successfully",
      data: config,
    });
  },
);
