...
const DateByfetchoutlets = asyncHandler(async (req, res) => {
  console.log("Fetching outlets...");

  if (!(await acquireLock("custom-syncOutletMaster"))) {
    console.log(
      "custom-syncOutletMaster: Lock not acquired, another sync in progress",
    );
    res.status(400);
    throw new Error("Another sync is in progress. Please try again later.");
  }
  console.log("✅ [custom-syncOutletMaster] Lock acquired.");
  ...
  try {
    ...
    console.log(`fetching outlet from ${StartDate} to ${EndDate}`);

    // Get ClientCode from query parameter (mandatory)
    ...
    if (!clientCode) {
      return res.status(400).json({
        error: true,
        message:
          "ClientCode is mandatory. Please provide ClientCode in query parameters.",
      });
    }

    // console.log(
    //   `Fetching the Order SKU Report API from tartDate} to ${endDate}`,
    // );
    ...
    for (const [phoneNumber, rows] of Object.entries(phoneNumberGroups)) {
     ...
     console.log(existingOutlet, "existingOutlet->>>>>>>>");
      if (existingOutlet) {
    ...
    await releaseLock("custom-syncOutletMaster");
    console.log("custom-syncOutletMaster: Lock released, finished");
  }
});
...
module.exports = { DateByfetchoutlets };
