...
const retailerLogin = asyncHandler(async (req, res) => {
  ...
  if (!outletApproved) {
    return res.status(400).json({
      error: true,
      message: "We couldn’t sign you in. The account either does not exist or has not been approved yet.",
    });
  }

  // Generate OTP
  const otpToSave = generateOTP();

  // Save login record
  const loginEntry = await RetailerLogin.create({
  ...
  const params = {
    ...
    Message: `Your PASSCODE for logging into the Rupa Pragati Application is ${otpToSave}`,
    Contacts: contact,
