...
const productListPaginatedForPurchaseReturn = asyncHandler(async (req, res) => {
  try {
    ...
    // NEW: godown selected on the Purchase Return screen — passed through
    // to inventory lookup below so "Available Qty" reflects that godown.
    const godownId = req?.query?.godownId || null;
    ...
    // NEW: godownId passed through so stock reflects the selected godown.
    // Utility already handles null/undefined by falling back to "main".
    const batchInventory = await getBatchInventoryStock(productIdsForInventory, distributorId, godownId);
...
module.exports = { productListPaginatedForPurchaseReturn };