...
const getBatchProductPricing = async (
  ...
  try {
    ...
    // If quotationDate is given, we check ranges against that date instead
    // of "now" — and we don't filter by status at all (active or not).
    const compareDate = quotationDate
      ? moment(quotationDate).tz("Asia/Kolkata").startOf("day").toDate()
      : nowDateTime;
    ...
    const priceList = await Price.find(priceQuery)
      .populate([
    ...
    // Step 3: Fetch distributor and region info if needed
    let distributor = null;
    let region = null;
    ...
    // Step 4: Process pricing for each product
    const result = {};
    ...
    productIds.forEach((productId) => {
      ...
      const isPriceValid = (price) => {
        ...
        if (price?.expiresAt) {
          ...
          return (
            moment(effectiveDate).isSameOrBefore(compareDate) &&
            moment(expiresAt).isSameOrAfter(compareDate)
          );
        } else {
          return moment(effectiveDate).isSameOrBefore(compareDate);
        }
      };
