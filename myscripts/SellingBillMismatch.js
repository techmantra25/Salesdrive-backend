/**
 * Script to track mismatches between selling bill line items count and
...
function getCountableLineItems(bill) {
  ...
  return (bill.lineItems || [])
    ...
    .map((item) => ({
      ...
      reason:
        Number(item.billQty || 0) <= 0
          ? "Zero billQty"
          : `Non-countable itemBillType: ${item.itemBillType}`,
    }));
}
...
async function buildMismatchDetail(bill, transactions) {
  const lineItems = getCountableLineItems(bill);
  ...
  const missingProducts = lineItems
    ...
    .map((lineItem) => {
      ...
      return {
        ...
        transactionId: transactionIds.length > 0 ? transactionIds.join(", ") : "N/A",
      };
    });
  ...
  return {
    ...
    difference: lineItems.length - transactions.length,
...
async function processDistributorBills(distributor) {
  ...
  try {
    ...
    for (const bill of bills) {
      const lineItemsCount = getCountableLineItems(bill).length;
...
function printSummary() {
  log(`\n${"=".repeat(80)}`);
  log("SELLING BILL LINE ITEMS VS DELIVERY TRANSACTIONS MISMATCH REPORT");
  log(`${"=".repeat(80)}`);
...
function writeJSONReport() {
  ...
  const filename =
    OUTPUT_FILE || `SellingBillMismatch_report_${timestamp}.json`;
...
async function main() {
  log(`
========================================================================
      Selling Bill Line Items vs Delivery Transactions Counter Script
========================================================================
`);
